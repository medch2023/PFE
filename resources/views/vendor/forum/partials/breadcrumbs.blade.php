<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><!--<a href="{{ url(config('forum.web.router.prefix')) }}">{{ trans('forum::general.index') }}</a>--><a class="breadcrumb-item" href="/forum">Forum</a></li>
        ...
        @if (isset($breadcrumbs_append) && count($breadcrumbs_append) > 0)
            @foreach ($breadcrumbs_append as $breadcrumb)
                <li class="breadcrumb-item">Discussions récentes</li>
            @endforeach
        @endif
    </ol>
</nav>
